...
export default class CreateAreaRectangleTool extends EventEmitter {
    ...
    mouseMoveHandler(e) {
        ...
        /*
            Proposed new changes for collision detection
            by Calvin Feng
         */
        // children = [freeAreaGroup, keepoutAreaGroup, lanesGroup, guideLines]
        let isCollidingWithOtherViews = false;
        for (let i = 0; i < this.realWorldCoordinatesGroup.children.length; i++) {
            if (i === 0) {
                let freeAreaGroup = this.realWorldCoordinatesGroup.children[i];
                freeAreaGroup.children.forEach((freeAreaView) => {
                   freeAreaView.children.forEach((path) => {
                       this.areaView.viewGroup.children.forEach((thisPath) => {
                           if (thisPath.getIntersections(path).length !== 0) {
